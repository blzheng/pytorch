...
from torchdynamo.profiler import fx_insert_profiling, Profiler
from torchdynamo.testing import dummy_fx_compile, format_speedup, same
from torchdynamo.utils import clone_inputs
from torchinductor import config as inductor_config
from torchinductor.utils import fresh_inductor_cache
...
def maybe_init_distributed(should_init_distributed, port="6789", rank=0, world_size=1):
    # To avoid multiple inheritance from dynamo.test_case.TestCase and MultiProcessTestCase,
...
def run(runner, args, original_dir=None):
    ...
    torchdynamo.config.suppress_errors = args.suppress_errors
    ...
    if args.overhead:
        ...
        if args.backend == "ipex":
            if args.float32:
                optimize_ctx = torchdynamo.optimize(backends.ipex_fp32)
            elif args.amp:
                optimize_ctx = torchdynamo.optimize(backends.ipex_bf16)
        else:
